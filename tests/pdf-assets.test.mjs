...
test('repeated PDF asset preparation leaves unchanged files untouched and repairs changed or missing assets', async t => {
  ...
  const sources = [
    'pdfjs-dist/LICENSE', 'tesseract.js/LICENSE.md', 'tesseract.js-core/LICENSE',
  ...
  for (const path of files.filter(path => !['worker.min.js', 'wasm/nested/decoder.wasm'].includes(path))) {
